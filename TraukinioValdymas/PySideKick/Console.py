...
class _QPythonConsoleUI(object):
    ...
    def __init__(self,parent):
        ...
        self.output = QtGui.QTextEdit(parent)
...
class QPythonConsole(QtGui.QWidget):
	...
	self.onText.connect(self.writeText)
    ...
    def write(self, text):
      self.onText.emit(text)
    
    def writeText(self, text):   
      #self.lock.lock()
      
      cursor = QtGui.QTextCursor(self.ui.output.textCursor())
      fmt = QtGui.QTextCharFormat()
	...
	cursor.insertText(text)
      ...
      self.ui.output.moveCursor(QtGui.QTextCursor.End, QtGui.QTextCursor.MoveAnchor)
